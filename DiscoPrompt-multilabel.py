...
from data_utils import PDTB2Processor, PDTB3Processor, CoNLL15Processor, DiscogemMultilabelProcessor
...
assert args.lang in ['en', 'de', 'fr', 'cs']
...
#     }
    ...
    num_classes = 17
    Processor = DiscogemMultilabelProcessor(num_labels=num_classes)
    data_dir = os.path.join('/nethome/peterb/DiscoGeM/discogem-multilabel', args.lang)
    dataset = {}
    dataset['train'] = Processor.get_examples(data_dir, 'train')
    dataset['validation'] = Processor.get_examples(data_dir, 'dev')
    dataset['test'] = Processor.get_examples(data_dir, 'test')
    ...
    label_words = {
        ...
        "Expansion.Substitution": ['instead', 'Expansion', 'Substitution'],
        "Contingency.Purpose": ['in order', 'Contingency', 'Purpose'],
        "Expansion.Manner": ['in this way', 'Expansion', 'Manner'],
        "Expansion.Disjunction": ['alternatively', 'Expansion', 'Disjunction'],
        "Contingency.Negative-Condition": ['otherwise', 'Contingency', 'Negative-Condition'],
        "Contingency.Condition": ['if', 'Contingency', 'Condition'],
        "Expansion.Exception": ['except', 'Expansion', 'Exception']
    }
